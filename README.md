...
- 增加接同传接口的mode参数：可选择翻译方式为“语音转语音”或“语音转文字”。默认为“语音转文字”
  > 选择语音转文字模式时仅显示文字字幕，不响应语音流。接口单价更便宜。（参考价：输出文字￥80/百万Token，输出语音￥300/百万Token）
- 字幕窗口可选择隐藏原文栏 和 系统声音栏。（此功能有样式BUG，若选择隐藏系统声音栏，每次重启软件都需要重设一次。不打算修改了。）
...
使用火山引擎同声传译 2.0 AST 服务时，需要在控制台获取 `API Key` ：

https://console.volcengine.com/speech/service/10030

目前申请服务会有百万Token的免费体验额度。（约70万字）

在软件设置中填写到语音服务页的火山 AST 凭据区：

- `APP ID` -> `X-Api-Key`

注意：这里不要填写账号级 IAM AK/SK，这些语音接口使用的是语音服务页面里的 `API Key`。
