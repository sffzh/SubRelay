...
GameSubRelay 主链路使用火山引擎同声传译 2.0 AST 服务。AST 同时承担识别、翻译、字幕输出和可选目标语音输出。
...
当前实现已经支持麦克风 AST `s2s`。下一步需要把游戏/系统声音从纯 ASR 改为 AST `s2t`，让游戏字幕主链路也具备翻译能力。
